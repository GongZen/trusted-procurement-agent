...
공공데이터포털 농수산식품기업지원센터(B552845) 오픈API. 7개 조건으로 검증한 13종 가운데 4종을 썼다 — 유통 3단계와 오늘 시세 감지에 필요한 최소 집합이다. 나머지는 지역별·기간별·친환경 같은 보조 소스라 MVP에 넣지 않았다. 후보 전체와 판정 근거는 [DATA_CRITERIA §2](DATA_CRITERIA.md)에 있다.
...
제3자가 자기 키로 같은 데이터를 받을 수 있다 ([DATA_SOURCES.md](DATA_SOURCES.md)).
