...
def 판정하기(설정: dict, 기준일: str, 소매행: list[dict],
    ...
    결과.sort(key=lambda p: (-집계[p.품목]["최고도시비율"],
                           -집계[p.품목]["중앙배수"], p.품목))

    # 🔴 **자르지 않는다.** 전에는 여기서 상위 5개를 잘랐는데, 그 5 는
    #    우리가 고른 값이었다. 「상위 5개만 보여준다고 밝힌다」는 숨김을
    #    공개할 뿐 자르는 숫자를 없애지 못한다(PR #14 검토 지적).
    #
    #    대신 **「최근 5년 같은 달 최고가인 도시가 1곳이라도 있는가」**로
    #    고른다. 예/아니오라 우리가 고를 값이 없고, 몇 개가 나올지는
    #    그날의 자료가 정한다. **0개면 아무것도 안 나온다** — 그게
    #    「오늘은 볼 것 없다」의 정확한 의미다.
    #
    #    화면은 캐러셀이라 길어져도 무너지지 않는다.
    올릴것 = [p for p in 결과 if 집계[p.품목]["최고도시수"] > 0]
