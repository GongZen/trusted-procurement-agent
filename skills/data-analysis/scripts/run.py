...
def 판정하기(설정: dict, 기준일: str, 소매행: list[dict],
    ...
    for 품목, d in 집계.items():
        ...
        # 🔴 카드 왼쪽 큰 숫자는 **주인공 단계**의 것이다. 전에는 소매 세부
        #    구간(서울 깻잎 무농약 · 친환경농산물)을 크게 썼는데, 그것만
        #    카드의 축과 따로 놀았다. 지역×품종×등급을 이어 붙인 이름도
        #    따로 노는 값들의 나열이라 의미를 만들지 못했다.
        큰숫자 = None
        함께 = []
        if 주인공:
            s = next(x for x in 단계 if x.단계 == 주인공)
            if s.올해 and s.평년평균:
                율 = round((s.올해 / s.평년평균 - 1) * 100)
                큰숫자 = {
                    "단계": 주인공,
                    "이름": {"산지": "전국 산지공판장 낙찰가",
                           "도매": "도매시장 중도매 가격",
                           "소매": "소매 판매가"}.get(주인공, 주인공),
                    "단위": s.단위, "평년": s.평년평균, "올해": s.올해,
                    "증감": f"{'+' if 율 >= 0 else ''}{율}%"}
            # 주인공 말고도 1번째인 단계가 있으면 문장이 그것을 말한다
            함께 = [x.단계 for x in 단계
                  if x.단계 != 주인공 and (x.순위 or "").split("/")[0] == "1"]
        ...
        결과.append(analyze.품목판정(
            ...
            사람말=(analyze.주인공말(
                        next(s for s in 단계 if s.단계 == 주인공), 대상월, 함께)
                  + " 소매에서는 "
