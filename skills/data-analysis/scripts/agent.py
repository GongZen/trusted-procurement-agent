...
def main() -> None:
    ...
    for 회차 in range(1, 반복 + 1):
        ...
        # 🔴 **띄울 파일을 출력에 직접 적는다.**
        #    타임리에서 모델이 이 파일을 쓰지 않고 **자기가 HTML 을 새로
        #    만들었다.** 그 화면은 품목 이름을 「족발」로 바꿔 놓았는데
        #    숫자는 풋고추 것이었다 — 오류가 아니라 조용한 날조다.
        #    지시를 프롬프트에만 두지 않고 **도구 출력에 함께 실어** 보낸다.
        if 결과.산출:
            화면 = OUT / "report.html"
            if 화면.exists():
                print(f"\n▶ 띄울 화면 {화면} ({화면.stat().st_size/1024:.0f}KB)")
                print("  이 파일을 그대로 띄운다. HTML 을 새로 만들지 않는다.")
                print("  사람이 읽을 요약이 필요하면 report.txt 를 그대로 옮긴다.")
        elif 결과.상태 == "변화없음":
            # 🔴 실측에서 모델이 「변화 없음」인데도 아티팩트를 다시 만들었다.
            #    프롬프트의 금지만으로는 「도움이 되려는」 기본 행동을 못 이긴다.
            #    **하지 말 것을 도구 출력에 직접 적는다.**
            print("\n▶ 만들 화면이 없다. 이미 띄운 것이 최신이다.")
            print("  아티팩트를 다시 만들지 않는다. 판정도 다시 하지 않았다.")
        if 회차 < 반복 and 간격:
            time.sleep(간격)
