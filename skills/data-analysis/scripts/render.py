...
def 고른범위(보고서: dict) -> str:
    ...
    return (f"판정된 <b>{검증.get('판정품목', '?')}종</b> 중 "
            f"<b>상위 {len(보고서.get('우선검토', []))}종</b>을 올립니다")
...
def 판정제외(보고서: dict) -> str:
    ...
    문장 = [f"과거 같은 달 관측이 <b>{최소}개 미만</b>인 구간은 판정하지 않습니다"
           ...
           "관측 2개로 순위를 매기면 「2년 중 1번째」가 되어 "
           "<b>말이 관측보다 커지기</b> 때문입니다."]
    if 없음:
        문장.append(f"올해 같은 달 관측이 아직 없는 구간 {없음:,}개도 뺐습니다.")
    문장.append(f"<b>이 관측 수({최소}개)와 위의 다섯 종, 둘이 저희가 고른 값의 "
               f"전부입니다.</b> 가격의 높낮이를 가르는 선은 쓰지 않습니다.")
    return " ".join(문장)
...
def 텍스트로(보고서: dict) -> str:
    ...
    검증 = 보고서.get("검증", {})
    제외 = 검증.get("제외구간") or {}
    줄.append(f"  판정된 {검증.get('판정품목', '?')}종 중 상위 {len(판정들)}종을 올렸습니다."
