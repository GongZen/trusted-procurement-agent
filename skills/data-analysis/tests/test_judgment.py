...
def main() -> None:
    ...
    # ── ⑧ 우리가 고른 수를 숨기지 않는다 ────────────────────────────
    print("\n⑧ 우리가 고른 수(최소관측수·표시 품목수)를 산출물이 밝힌다")
    검증 = 보고.get("검증", {})
    확인(검증.get("최소관측수") and "제외구간" in 검증 and 검증.get("표시품목수"),
        f"판정 JSON 에 남는다 — 최소관측 {검증.get('최소관측수')}개 · "
        f"표시 {검증.get('표시품목수')}종 · 제외 {검증.get('제외구간')}")

    화면 = (paths.OUTPUT / "report.html").read_text(encoding="utf-8")
    확인("판정하지 않은 것" in 화면
        and f"{검증.get('최소관측수')}개 미만" in 화면
        and f"판정된 <b>{검증.get('판정품목')}종</b>" in 화면,
        "화면 가이드가 분모와 제외 기준을 함께 보여준다")

    글 = (paths.OUTPUT / "report.txt").read_text(encoding="utf-8")
    확인(f"판정된 {검증.get('판정품목')}종 중 상위" in 글,
        "텍스트 산출물에도 분모가 남는다")
