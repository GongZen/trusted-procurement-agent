...
| | 무엇 | 사람이 못 하는 이유 |
|---|---|---|
| **④** | **전 조합 탐색 → 상쇄 구간 탐지** | 조합 수가 사람의 인지 한계를 넘는다. 재무팀은 제품군 10~20개 단위로 볼 수밖에 없다 |
...
> **집계는 상쇄를 숨긴다.** 전사 마진이 −0.3%로 보고돼 별일 아닌 것처럼 보여도, 특정 고객군 × 특정 제품 조합에서 −12%가 진행 중이고 다른 조합의 +11%가 그것을 가리고 있을 수 있다. **④가 이 프로젝트의 값어치가 나오는 지점이다.**
