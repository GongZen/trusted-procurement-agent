...
| ④ | **🔑 상쇄 탐지** | 집계에서 **서로 상쇄되어 보이지 않던 구간**을 찾아낸다 | Step 2 · D2 |
...
> **집계는 상쇄를 숨긴다.** 방향이 반대인 변화는 합산 과정에서 서로를 지운다. 전사 지표가 완만해 보여도 특정 조합에서 큰 폭의 악화가 진행 중이고, 다른 조합의 개선이 그것을 가리고 있을 수 있다. **④가 이 프로젝트의 값어치가 나오는 지점이다.**
>
> ⚠️ 이 현상이 두 공개 데이터에서 **실제로 어느 정도 규모로 나타나는지는 아직 측정하지 않았다.** Step 2 완료 후 실측치로 대체한다. 그전까지 구체적 수치를 예시로 쓰지 않는다.
