...
- **유통 3단계가 같은 품목 코드를 쓴다.** 대분류 8개 전부 일치, (대·중·소) 조합 이름 불일치 0
...
| 순서 | 결정 | 왜 그 순서인가 |
|---:|---|---|
| 1 | **「이상」 판정 기준** | `yy1_bfr_cmpr_rafrt`(전년 대비)와 변동계수 중 무엇으로, 몇을 임계로. **임의 임계치를 쓰지 않는 방법**이어야 한다 |
| 2 | **심화 조사 분기 규칙** | 어떤 관측에서 어느 도구를 쓸지. Agent다움이 여기서 갈린다 |
| 3 | **수집 범위 최종** | 품목 수 × 기간 × 갱신 주기. 일 트래픽 10,000 안에 |
| 4 | Skill 입출력 계약 | 위 셋이 정해져야 필드가 잠긴다 |
