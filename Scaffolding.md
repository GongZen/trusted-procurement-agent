...
| 예외 | **D13** (플랫폼 연결) | 킥오프(7/28) 직후. **Step 5b 착수 전**까지 |

D13만 예외인 이유는 타임리 사양이 킥오프 전까지 공개되지 않기 때문이다.
그래서 S2(플랫폼 연결부를 얇게)가 중요하고, **Step 5를 5a(플랫폼 비의존)와 5b(플랫폼 연결)로 나눈다**(§3).
...
>
> **D12(성능)·D13(플랫폼 연결)은 위 그래프에 없다 — 다른 결정에 의존하지 않는 독립 결정이기 때문이다.**
> 잠금 시점은 **D12는 본체와 함께 7/27까지**(Step 7에서 쓰지만 앞당겨 잠근다), **D13만 Step 5b 착수 전**이다.
...
**Step 0 산출물** — 분석 사양서 1개 + `DECISIONS.md` 항목 12건 (D13은 킥오프 후 추가)
...
| 시점 | 하는 일 |
|---|---|
| ~7/27 | **Step 0(D1~D12 잠금) → Step 1 → 2 → 3 → 4 → 5a**까지 끝낸다 |
