...
flowchart TD
    ...
    D10 --> D13["D13 플랫폼 연결<br/>(잠금 시점만 예외)"]
    D12["D12 성능 전략<br/>(독립)"]
```
...
>
> **D13(플랫폼 연결)은 독립이 아니다 — D10에 의존한다.** 플랫폼에 얹을 트리거·재판정 규칙 자체가 D10에서 나오고, D13의 확정 조건에 *"Step 5a의 코어를 수정하지 않고 연결 가능함"*이 들어 있기 때문이다. **D13이 예외인 것은 의존 관계가 아니라 잠금 시점**이다(킥오프 이후에만 확정 가능).
